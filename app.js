...
passport.use(new GoogleStrategy({
    ...
    callbackURL: "https://mighty-ridge-15071.herokuapp.com/auth/google/secrets",
    scope:"profile"
  },
...
.get((req,res)=>{
   User.find({"secret":{$ne:null}}, (err,items)=>{
    if (err) {
        ...
        res.render("secrets",{usersWithSecrets:items})
    }
   });
});
...
.post((req,res)=>{
    ...
    User.findById(req.user.id,(err,item)=>{
        if (err) {
            console.log(err);
            
        } else {
            item.secret=postSecret;
            item.save((err)=>{
                if(!err){
